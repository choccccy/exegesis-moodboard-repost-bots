...
2. Someone reacts with 🦋 → the bot creates a `submission`.
...
4. For anything missing it posts one procedural reply per gap:
...
5. The original poster **or any curator-role user** replies; the bot records the answer
   and re-evaluates. When all requirements are met the submission reaches `ready_to_queue`.
