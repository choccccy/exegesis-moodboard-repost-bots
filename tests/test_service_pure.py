...
from bot.curation.statemachine import _DISCORD_MAX_BYTES, _alt_preview_for, _image_status, _primary_link, _queue_action
from bot.curation.handlers import _triage_relative
...
def test_animated_gif_falls_back_to_static_jpeg_when_nothing_fits(tmp_path):
    ...
    with patch("bot.curation.statemachine._DISCORD_MAX_BYTES", 1):
        with Image.open(path) as img:
            f = _discord_file_for_animated_gif(img, "anim.gif")
