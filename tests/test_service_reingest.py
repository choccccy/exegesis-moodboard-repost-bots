...
async def _reingest(session, sub, inbound, meta=None):
    with (
        ...
        patch("bot.curation.ingest.resolve", new_callable=AsyncMock, return_value=meta or _meta()),
        patch("bot.curation.ingest.download_attachment", new_callable=AsyncMock, return_value="/fake/f.jpg"),
        patch("bot.discord_ingest.service.session_scope", bound_session_scope(session)),
        patch("bot.curation.ingest.session_scope", bound_session_scope(session)),
    ):
