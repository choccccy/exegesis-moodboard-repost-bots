...
async def test_on_interaction_ignores_non_component(repost_bot):
    ...
    with patch("bot.curation.handlers.handle_confirm_button", new_callable=AsyncMock) as handler:
...
async def test_on_interaction_edit_prefix_routes_without_defer(repost_bot, session):
    ...
    with (
        ...
        patch("bot.curation.handlers.handle_edit_button", new_callable=AsyncMock) as handler,
    ):
        await repost_bot.on_interaction(interaction)
...
async def test_on_interaction_alt_prefixes_route_without_defer(
    ...
    with (
        patch("bot.discord_ingest.client.session_scope", bound_session_scope(session)),
        patch(f"bot.curation.handlers.{handler_name}", new_callable=AsyncMock) as handler,
    ):
        await repost_bot.on_interaction(interaction)
    handler.assert_awaited_once()
...
async def test_on_interaction_unknown_custom_id_is_ignored(repost_bot, session):
    ...
    with (
        patch("bot.discord_ingest.client.session_scope", bound_session_scope(session)),
        patch.multiple(
            "bot.curation.handlers",
            **{name: AsyncMock() for name in handler_names},
        ),
    ):
        import bot.curation.handlers as service_mod
...
async def test_reaction_add_special_emoji_unresolvable_channel_skips(
    ...
    with patch(f"bot.curation.handlers.{handler_name}", new_callable=AsyncMock) as handler:
        await repost_bot.on_raw_reaction_add(payload)
    handler.assert_not_awaited()
...
async def test_reaction_add_source_cancel_invokes_handler(repost_bot, session):
    ...
    with (
        patch("bot.discord_ingest.client.session_scope", bound_session_scope(session)),
        patch("bot.curation.handlers.handle_source_cancel_reaction",
...
async def test_reaction_add_cancel_in_thread_invokes_handler(repost_bot, session):
    ...
    with (
        patch("bot.discord_ingest.client.session_scope", bound_session_scope(session)),
        patch("bot.curation.handlers.handle_cancel_reaction", new_callable=AsyncMock) as handler,
    ):
        await repost_bot.on_raw_reaction_add(payload)
    handler.assert_awaited_once()
    assert handler.await_args.args[1].message_id == payload.message_id  # ReactionEvent
...
async def test_reaction_remove_trigger_routes_to_cleanup(repost_bot, session):
    ...
    with (
        patch("bot.discord_ingest.client.session_scope", bound_session_scope(session)),
        patch("bot.curation.handlers.handle_reaction_removed", new_callable=AsyncMock) as handler,
    ):
...
async def test_reaction_remove_unresolvable_channel_skips(repost_bot):
    payload = make_reaction_payload(channel_id=100)
    repost_bot._resolve_channel = AsyncMock(return_value=None)
    with patch("bot.curation.handlers.handle_reaction_removed", new_callable=AsyncMock) as handler:
        await repost_bot.on_raw_reaction_remove(payload)
    handler.assert_not_awaited()
...
async def test_on_message_ignores_own_messages(repost_bot):
    ...
    with (
        _patch_user(42),
        patch("bot.curation.handlers.handle_reply", new_callable=AsyncMock) as handler,
    ):
...
async def test_on_message_non_reply_ignored(repost_bot):
    ...
    with patch("bot.curation.handlers.handle_reply", new_callable=AsyncMock) as handler:
        await repost_bot.on_message(message)
    handler.assert_not_awaited()
