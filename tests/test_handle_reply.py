...
These use a real in-memory SQLite database (via the shared session fixture) and
drive the surface-agnostic handler with a normalized ReplyEvent + an in-memory
MockDest surface. They cover:
...
from bot.curation.handlers import handle_reply
...
def _reply_event(
    *,
    ...
    return ReplyEvent(
        bot_message_id=reply_to_id,
        author_id=author_id,
        member=member,
        message=InboundMessage(content=content, attachments=attachments or []),
    )


def _image_attachment(filename: str = "robot.jpg") -> InboundAttachment:
    return InboundAttachment(
        id=next(_NEXT_ID),
        url=f"https://cdn.discord.com/{filename}",
...
async def _reply(session, event, dest, settings):
    return await handle_reply(session, event, dest, settings, MagicMock())


async def _add_source_request(session, submission: Submission) -> SourceRequest:
    req = SourceRequest(submission_id=submission.id, bot_message_id=next(_NEXT_ID))
    session.add(req)
    await session.flush()
    return req
...
async def _add_alt_text_request(session, submission: Submission, attachment_id: int) -> AttachmentAltTextRequest:
    req = AttachmentAltTextRequest(
        submission_id=submission.id, attachment_id=attachment_id, bot_message_id=next(_NEXT_ID),
    )
    session.add(req)
    await session.flush()
    return req
...
async def _add_supplemental_request(session, submission: Submission) -> SupplementalImageRequest:
    req = SupplementalImageRequest(submission_id=submission.id, bot_message_id=next(_NEXT_ID))
    session.add(req)
    await session.flush()
    return req
...
async def test_handle_reply_unauthorized_user_ignored(session, board):
    ...
    event = _reply_event(reply_to_id=req.bot_message_id, author_id=999)  # neither OP nor curator
...
async def test_handle_reply_op_is_always_authorized(session, board):
    ...
    with patch("bot.curation.ingest._resolve_links_in_session", new=AsyncMock()):
        result = await _reply(session, event, MockDest(), settings)
...
async def test_handle_reply_curator_by_role_is_authorized(session, board):
    ...
    curator_role.id = 555
    event = _reply_event(
        reply_to_id=req.bot_message_id, author_id=999,
        content="https://example.com/post", roles=[curator_role],
    )
...
async def test_handle_reply_alt_text_empty_nudges(session, board):
    ...
    await session.flush()
    att = Attachment(
        submission_id=sub.id, discord_attachment_id=next(_NEXT_ID), filename="robot.jpg",
        discord_url="https://cdn.discord.com/robot.jpg", is_image=True,
    ...
    event = _reply_event(reply_to_id=req.bot_message_id, author_id=sub.author_id, content="   ")
...
async def test_handle_reply_alt_text_stored(session, board):
    ...
    dest = MockDest()
    with patch("bot.curation.statemachine.recompute_and_request", new=AsyncMock()):
    ...
    assert not any("updated" in s for s in dest.sent)
...
async def test_handle_reply_alt_text_overwrite_posts_notice(session, board):
    ...
    att = Attachment(
        submission_id=sub.id, discord_attachment_id=next(_NEXT_ID), filename="robot.jpg",
        discord_url="https://cdn.discord.com/robot.jpg", is_image=True,
        alt_text_status=AltTextStatus.PROVIDED.value, alt_text_body="old alt", alt_text_author=sub.author_id,
    )
    session.add(att)
    await session.flush()
    req = await _add_alt_text_request(session, sub, att.id)
    ...
    assert result is True
    fresh_att = await session.get(Attachment, att.id)
    assert fresh_att.alt_text_body == "a much better description"
    # A visible overwrite notice was posted, naming the file and the previous value.
    assert any("robot.jpg" in n and "updated" in n and "old alt" in n for n in dest.sent)
...
async def test_handle_reply_alt_text_same_value_no_notice(session, board):
    ...
    att = Attachment(
        submission_id=sub.id, discord_attachment_id=next(_NEXT_ID), filename="robot.jpg",
        discord_url="https://cdn.discord.com/robot.jpg", is_image=True,
        alt_text_status=AltTextStatus.PROVIDED.value, alt_text_body="existing alt text",
    ...
    assert result is True
    assert not dest.sent  # no overwrite notice since the value didn't change
...
async def test_handle_reply_source_url_creates_link(session, board):
    ...
    with patch("bot.curation.ingest._resolve_links_in_session", new=AsyncMock()), \
         patch("bot.curation.statemachine.recompute_and_request", new=AsyncMock()):
        result = await _reply(session, event, MockDest(), _mock_settings())
...
async def test_handle_reply_source_non_url_offers_confirmation(session, board):
    ...
    assert result is True
    assert len(dest.sent) == 1  # source-note confirmation prompt
...
async def test_handle_reply_supplemental_image_ingests_images(session, board):
    ...
    with patch("bot.curation.ingest._ingest_attachment_in_session", new=AsyncMock()) as mock_ingest, \
         patch("bot.curation.statemachine.recompute_and_request", new=AsyncMock()):
        result = await _reply(session, event, MockDest(), _mock_settings())
...
async def test_handle_reply_supplemental_image_no_image_nudges(session, board):
    ...
    pdf_att = InboundAttachment(
        id=next(_NEXT_ID), filename="document.pdf", content_type="application/pdf",
    )
    event = _reply_event(
        reply_to_id=req.bot_message_id, author_id=sub.author_id, attachments=[pdf_att],
    )
    dest = MockDest()
    result = await _reply(session, event, dest, _mock_settings())
...
async def test_handle_reply_already_answered_ignored(session, board):
    ...
    event = _reply_event(
        reply_to_id=req.bot_message_id, author_id=sub.author_id, content="https://new.example.com",
    )
    result = await _reply(session, event, MockDest(), _mock_settings())

    assert result is True  # recognized as ours but not reprocessed
    links = list(await session.scalars(
        select(SubmissionLink).where(SubmissionLink.submission_id == sub.id)
    ))
    assert len(links) == 0
...
async def test_handle_reply_unknown_bot_message_returns_false(session, board):
    ...
    event = _reply_event(reply_to_id=99999, author_id=sub.author_id)
    result = await _reply(session, event, MockDest(), _mock_settings())
    assert result is False


async def test_handle_reply_orphaned_request_returns_false(session, board):
    # The request row exists but its submission is gone (deleted mid-flight).
    req = SourceRequest(submission_id=987_654, bot_message_id=next(_NEXT_ID))
    session.add(req)
    await session.flush()

    event = _reply_event(reply_to_id=req.bot_message_id, author_id=1, content="https://x.com/y")
    result = await _reply(session, event, MockDest(), _mock_settings())
    assert result is False
...
async def _add_metadata_request(session, submission: Submission) -> MetadataRequest:
    req = MetadataRequest(submission_id=submission.id, bot_message_id=next(_NEXT_ID))
    session.add(req)
    await session.flush()
    return req
...
async def _add_supplemental_link_request(session, submission: Submission) -> SupplementalLinkRequest:
    req = SupplementalLinkRequest(submission_id=submission.id, bot_message_id=next(_NEXT_ID))
    session.add(req)
    await session.flush()
    return req
...
def _add_primary_link(session, submission: Submission) -> SubmissionLink:
    link = SubmissionLink(
        submission_id=submission.id, order_index=0,
        raw_url="https://old.example.com/post", canonical_url="https://old.example.com/post",
        domain_family="other", resolved_title="Old Title", resolved_description="Old description",
        resolved_image_url="https://old.example.com/img.jpg", resolved_image_path="/vol/old.jpg",
...
async def test_handle_reply_metadata_url_replaces_primary_link(session, board):
    ...
    dest = MockDest()
    with patch("bot.curation.ingest._resolve_links_in_session", new=AsyncMock()) as mock_resolve, \
         patch("bot.curation.statemachine.recompute_and_request", new=AsyncMock()):
        result = await _reply(session, event, dest, _mock_settings())
    ...
    assert dest.sent  # "link updated" ack
...
async def test_handle_reply_supplemental_link_no_url_nudges(session, board):
    ...
    event = _reply_event(
        reply_to_id=req.bot_message_id, author_id=sub.author_id, content="no link here",
    )
    dest = MockDest()
    result = await _reply(session, event, dest, _mock_settings())
