...
from ..curation import base, handlers, ingest, statemachine
...
class RepostBot(discord.Client):
    ...
    async def setup_hook(self) -> None:
        ...
        async with session_scope() as session:
            await handlers.sync_boards(session, self.settings)
    ...
    async def on_interaction(self, interaction: discord.Interaction) -> None:
        ...
        if custom_id.startswith("edit:"):
            ...
            async with session_scope() as session:
                outcome = await handlers.handle_edit_button(session, event, self.settings)
        ...
        if custom_id.startswith("alt_edit:"):
            ...
            async with session_scope() as session:
                outcome = await handlers.handle_alt_edit_button(session, event, self.settings, self._yt_client)
            await gateway.perform(interaction, outcome)
            return
        if custom_id.startswith("alt_pick:"):
            event = gateway.to_event(interaction, int(custom_id.removeprefix("alt_pick:")))
            async with session_scope() as session:
                outcome = await handlers.handle_alt_pick(session, event, self.settings, self._yt_client)
            await gateway.perform(interaction, outcome)
            return
        ...
        async with session_scope() as session:
            if custom_id.startswith("cancel:"):
                outcome = await handlers.handle_cancel_button(
                ...
                outcome = await handlers.handle_confirm_button(
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent) -> None:
        ...
        if emoji == GRAPHIC_YES_EMOJI:
            ...
            async with session_scope() as session:
                await handlers.handle_label_reaction(
        ...
        if emoji == replies.METADATA_CONFIRM_EMOJI:
            ...
            surface = gateway.surface_for_channel(channel, self)
            async with session_scope() as session:
                await handlers.handle_metadata_reaction(
                    session, gateway.to_reaction_event(payload), surface,
                    self.settings, yt_client=self._yt_client,
                )
        ...
        if emoji == CANCEL_EMOJI:
            ...
            if payload.channel_id in self._watched_channels:
                ...
                async with session_scope() as session:
                    await handlers.handle_source_cancel_reaction(
                        session, gateway.to_reaction_event(payload), surface,
                        self.settings, yt_client=self._yt_client,
                    )
            else:
                ...
                surface = gateway.surface_for_channel(channel, self)
                async with session_scope() as session:
                    await handlers.handle_cancel_reaction(
        ...
        if emoji == replies.PLAYLIST_OPT_OUT_EMOJI:
            ...
            surface = gateway.surface_for_channel(channel, self)
            async with session_scope() as session:
                await handlers.handle_playlist_opt_out(
                    session, gateway.to_reaction_event(payload), surface,
                    self.settings, yt_client=self._yt_client,
                )
        ...
        if emoji == replies.CONFIRMATION_EMOJI:
            ...
            surface = gateway.surface_for_channel(channel, self)
            async with session_scope() as session:
                await handlers.handle_confirmation_reaction(
                    session, gateway.to_reaction_event(payload), surface,
                    self.settings, yt_client=self._yt_client,
                )
    ...
    async def on_raw_reaction_remove(self, payload: discord.RawReactionActionEvent) -> None:
        ...
        async with session_scope() as session:
            await handlers.handle_reaction_removed(session, event, surface, self.settings)
    ...
    async def _handle_scan_slash(self, interaction: discord.Interaction, days: int) -> None:
        ...
        if not handlers._is_curator(member, interaction.user.id, board_cfg):
            await interaction.response.send_message(
    ...
    async def _handle_triage_slash(
        ...
        await interaction.response.defer(ephemeral=True)
        async with session_scope() as session:
            board = await handlers._board_for_channel(session, channel.id)
            if board is None:
                await interaction.followup.send("Board not found.", ephemeral=True)
                return
            items = await handlers.fetch_triage_items(
                session,
                board_id=board.id,
    ...
    async def _handle_status_slash(self, interaction: discord.Interaction) -> None:
        ...
        async with session_scope() as session:
            ...
            member = interaction.user if isinstance(interaction.user, discord.Member) else None
            if not handlers._reaction_authorized(member, interaction.user.id, submission, board_cfg):
                await interaction.response.send_message(
                    "You're not authorised to view this submission.", ephemeral=True
                )
                return

            content = await statemachine.render_submission_status(session, submission)
    ...
    async def _handle_reingest_slash(self, interaction: discord.Interaction) -> None:
        ...
        async with session_scope() as session:
            ...
            if not handlers._reaction_authorized(member, interaction.user.id, submission, board_cfg):
                ...
                return
            if submission.state in statemachine._QUEUE_TERMINAL:
    ...
    async def _handle_nosource_slash(self, interaction: discord.Interaction) -> None:
        ...
        async with session_scope() as session:
            ...
            if not handlers._reaction_authorized(member, interaction.user.id, submission, board_cfg):
                ...
                return
            waived = await handlers.waive_source(
    ...
    async def _handle_skipalt_slash(self, interaction: discord.Interaction) -> None:
        ...
        async with session_scope() as session:
            ...
            if not handlers._reaction_authorized(member, interaction.user.id, submission, board_cfg):
                ...
                return
            count = await handlers.skip_all_alt_text(
                session, submission, settings=self.settings, user_id=interaction.user.id,
                destination=channel, yt_client=self._yt_client,
            )
        await interaction.followup.send(
    ...
    async def _thread_surface_for_source(self, source_channel_id: int, source_message_id: int):
        ...
        async with session_scope() as session:
            board = await handlers._board_for_channel(session, source_channel_id)
            if board is None:
                return NullSurface()
        ...
        for thread_id, queued_at, board_id, source_msg_id, channel_id, playlist_skipped, state in queued:
            ...
            if state == SubmissionState.QUEUED.value:
                async with session_scope() as session:
                    ...
                    if not await statemachine._playlist_close_ready(
                        ...
                        if thread is not None:
                            async with session_scope() as session:
                                ...
                                if fresh is not None:
                                    await statemachine.recompute_and_request(fresh.id, settings=self.settings, destination=gateway.surface_for_channel(thread, self), bot_id=getattr(self.user, "id", None), ambient_session=session)
